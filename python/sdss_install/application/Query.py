...
class Query():
    ...
    def set_graphql_dir(self):
        ...
        try: self.graphql_dir = environ['SDSS_INSTALL_GRAPHQL_DIR']
            ...
            self.logger.error('Unable to set the directory ' +
                              'from the environmental variable %r.' % e)
            ...
            if self.graphql_dir:
                if self.parameters['pagination_flag']:
                    ...
                    self.file = join(self.graphql_dir,
                                     ...
                                     % self.parameters['query_file_name'])
            if not self.file:
                s = 'Unable to set the GraphQL query file.\n'
                s += 'graphql_dir = %r\n' % self.graphql_dir
                s += 'parameters = %s' % json.dumps(self.parameters, indent=1)
                self.logger.error(s)
        else: self.logger.error('Unable to set the GraphQL query file.\n' +
                                'parameters = %s' % json.dumps(self.parameters))
