...
class Repositories:
    ...
    def get_repository_names(self):
        ...
        if self.ready:
            ...
            repository_names = self.repositories if self.repositories else list()
    ...
    def set_repositories(self):
        ...
        if self.ready:
            ...
            if self.ready: self.set_repository_data()
            if self.ready: self.repositories.extend(self.repository_list)
            if self.ready:
    ...
    def set_repository_data(self):
        ...
        '''
        if self.ready: self.set_repository_payload()
        if self.ready: self.set_repository_edges_and_page_info()
        if self.ready: self.set_repository_list()

    def set_repository_payload(self):
        '''Set GraphQL repository query payload data.'''
        if self.ready:
            self.set_payload()
            if self.payload:
                self.repository_payload = self.payload
            else:
                self.ready = False
    ...
    def set_repository_edges_and_page_info(self,data=None):
        ...
        if self.ready:
            ...
            if data:
                ...
                self.ready = False
                self.logger.error('Unable to set_repository_edges_and_page_info.' +
                                  'data: {}'.format(data))
        ...
        if self.ready:
            if self.repository_edges:
                for repository in self.repository_edges:
                    repository_name = repository['node']['name'] if repository else None
                    if repository_name: self.repository_list.append(repository_name)
            else:
                self.ready = False
                self.logger.error('Unable to set_repository_list.' +
                                  'self.repository_edges: {}'.format(self.repository_edges))
